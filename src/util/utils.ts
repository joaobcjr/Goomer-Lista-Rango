import { DiaPromocao } from 'src/promocao/diaPromocao/diapromocao.entity';
...
export function formatar_promocao(Horarios: DiaPromocao[]): any {
  const horarios = [null, null, null, null, null, null, null];
  ...
  for (const e of Horarios) {
    ...
    horarios[e.dia_semana - 1] = {
      dia: dias[e.dia_semana - 1],
      horario: `Válida das: ${horario_inicio.substr(0, 2)}:${horario_inicio.substr(
        2,
        4,
      )} às ${horario_fim.substr(0, 2)}:${horario_fim.substr(2, 4)}`,
    };
  ...
  return horarios.map((e, i) => {
    if (e === null && horarios.length > 0)
      return { dia: dias[i], horario: `Não válida` };
    else return e;
  });
}

export function formatar_status(status: number): string {
